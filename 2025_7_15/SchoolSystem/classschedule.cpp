    ...
    , ui(new Ui::ClassSchedule)
{
    ...
    if (!localCache->openDatabase())    qDebug() << "缓存数据库打开失败!";

    dataBase = DatabaseManager::instance();
    if(!dataBase->openDatabase())    qDebug() << "云端数据库打开失败!";
    ...
    ui->comboBox_1->addItems(dataBase->getClassSchedule());   //导致缓存的那个major重复出现。
    ComboBox1to2();ComboBox2to3();ComboBox3to4();ComboBox4to5();
    ...
    connect(dataBase, &DatabaseManager::queryError, this, [](const QString& err){ qDebug() << "查询失败：" << err; });
}
...
{
    localCache->updateCachingData(ui->comboBox_1->currentText(),ui->comboBox_2->currentText(),ui->comboBox_3->currentText(),ui->comboBox_4->currentText(),ui->comboBox_5->currentText().toInt(),true);
    localCache->replaceCoursesTable(dataBase->getClassScheduleModel(ui->comboBox_1->currentText(),ui->comboBox_2->currentText(),ui->comboBox_3->currentText(),ui->comboBox_4->currentText(),ui->comboBox_5->currentText().toInt()));
    DataModel = localCache->getCoursesTableModel();
    ui->tableView->setModel(DataModel);
