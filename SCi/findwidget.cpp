    ...
    , ui(new Ui::FindWidget)
{
    ...
    // 异步获取区域数据
    dbManager->getAreaItems([this](const QStringList& areas) {
        onAreaItemsReceived(areas);
    });
}
...
void FindWidget::on_pushButton_clicked()
{
    ...
    dbManager->getEmptyRoom(
        ...
        [this](const QStringList& rooms) {
            onEmptyRoomResultsReceived(rooms);
        }
        );
}
...
void FindWidget::on_areaComboBox_currentTextChanged(const QString &arg1)
{
    ...
    DatabaseManager* dbManager = DatabaseManager::instance();
    dbManager->getBuildingItems(arg1, [this](const QStringList& buildings) {
        onBuildingItemsReceived(buildings);
    });
}
