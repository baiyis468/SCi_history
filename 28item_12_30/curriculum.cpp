    ...
    , ui(new Ui::Curriculum)
{
    ...
    connect(comboBox5, &QComboBox::textActivated, this, &Curriculum::onComboBox5IndexChanged);
...
void Curriculum::onComboBox5IndexChanged(const QString &text)
{
    if(comboBox4->isEnabled())
    {
        //如果班级框已经被启用了。。
        QString str = text;
        int weekint = str.remove(str.length() - 1, 1).toInt();

        model->updateModel(getCurriculum::getInstance().getwVector()[weekint]);
