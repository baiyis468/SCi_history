...
LocalCacheManager* LocalCacheManager::instance = nullptr;
...
void LocalCacheManager::closeDatabase() {
    db.close();
}
...
bool LocalCacheManager::saveCachingData(const QString& major, const QString& educationLevel,
                                        int grade, int classNum, int weekNum, bool updated) {
    QSqlQuery query;
...
QMap<QString, QVariant> LocalCacheManager::getCachingDataMap() {
    ...
    if (query.next()) {
        ...
        data["class_num"] = query.value("class_num");
