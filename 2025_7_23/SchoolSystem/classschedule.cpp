    ...
    , ui(new Ui::ClassSchedule)
{
    ...
    connect(dataBase, &DatabaseManager::queryError, this, [this](const QString& err){
        cloudSuccess = false;
        qDebug() << "云端查询失败：" << err;
    });}
...
void ClassSchedule::ComboBox5toG()
{
    ...
    // 更新缓存数据
    if (!localCache->updateCachingData(ui->comboBox_1->currentText(),
    ...
    // 从云端获取课程表模型
    QSqlQueryModel *cloudModel = dataBase->getClassScheduleModel(
    ...
    if (!cloudModel || cloudModel->rowCount() == 0) {
        cloudSuccess = false;
        qDebug() << "云端查询课程表失败";
    }

    // 替换本地课程表
    if (cloudModel && !localCache->replaceCoursesTable(cloudModel)) {
        qDebug() << "课程表替换失败";
        cacheSuccess = false;
    }
